...
def test_login(client, random_user_data):
    ...
    response = client.post(
        "/auth/login",
        data={
            "username": random_user_data[
                "email"
            ],  # ye yaha apn ne json ki jagah data isi liye kar dia kyuki apn ne login route me oauth2requestform laga rakha hai jo ki form data leta hai jso nahi
