...
def regestered_user(client,random_user_data):
    ...
    response = client.post("/auth/login", data={"username": random_user_data["email"],"password":random_user_data["password"]})
    token = response.json()["access_token"]  # this data fron fake server is coming in raw json format .json()convert it into dictionary 
